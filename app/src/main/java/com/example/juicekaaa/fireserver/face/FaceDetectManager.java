...
public class FaceDetectManager {
    ...
    public interface OnFaceDetectListener {
        void onDetectFace(FaceInfo[] infos, ImageFrame imageFrame);
    }
...
//        }
        ...
        if (useDetect) {
            ...
            FaceInfo[]faceInfos = FaceSDKManager.getInstance().getFaceDetector().trackMaxFace(frame);
            if (faceInfos != null) {
                Log.e("faceMulti", faceInfos.length + "");
            }

            Log.e("wtf", " process->" + (System.currentTimeMillis() - starttime));

            if (faceInfos!=null&&faceInfos.length>0) {
                faceFilter.filter(faceInfos, frame);
            }
            if (listener != null) {
                listener.onDetectFace(faceInfos, frame);
            }
        }
